...
{
    public abstract class ToolTask2 : Task, ICancelableTask, ITask
    {
        // Fields
        ...
        protected ToolTask2(ResourceManager taskResources)
            : this()
        {
            base.TaskResources = taskResources;
        }

        protected ToolTask2(ResourceManager taskResources, string helpKeywordPrefix)
            : this(taskResources)
        {
        ...
        private string GetTemporaryResponseFile(string responseFileCommands, out string responseFileSwitch)
        {
            ...
            if (!string.IsNullOrEmpty(responseFileCommands))
            {
                ...
                {
                    writer.Write(this.ResponseFileEscape(responseFileCommands));
                }
        ...
        private void KillToolProcessOnTimeout(Process proc, bool isBeingCancelled)
        {
            if (!proc.HasExited)
            {
                ...
                {
                    this.LogShared.LogWarningWithCodeFromResources("Shared.KillingProcessByCancellation", new object[] { proc.ProcessName });
                }
                try
                {
